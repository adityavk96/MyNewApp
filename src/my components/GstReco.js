...
const parseDDMMYYYY = (str) => {
  ...
  if (match) {
    ...
    let mm = parseInt(match, 10);
    let yyyy = parseInt(match, 10);
...
const extractMonthFromInvoiceNo = (invNo) => {
  ...
  if (match) {
    return `${match[1].charAt(0).toUpperCase() + match.slice(1).toLowerCase()}-${match}`;
  }

  match = invNo.match(/(\d{1,2})[-./](\d{2})/);
  if (match) {
    const mi = parseInt(match, 10) - 1;
    if (mi>=0 && mi<12) return `${months[mi]}-${match}`;
  }
...
export default function GSTReconciliation() {
  ...
  const handleFileUpload = (e,setData) => {
    ...
    r.onload=(ev)=>{ try{ setData(parseExcel(ev.target.result)); } catch{alert("Failed to read file");}};
  ...
  const processRawData=(data,source)=>
    data.map((item)=>{
      ...
      return {
        ...
        IGST:toNum(item.IGST),CGST:toNum(item.CGST),
        SGST:toNum(item.SGST),Cess:toNum(item.Cess),
        Invoice_Date:date,
  ...
  const performReconciliation=(books,twoB)=>{
    ...
    books.forEach(b=>{
      map.set(b.recoKey,{
        GSTIN:b.GSTIN||"",Supplier_Name:b.Supplier_Name||"",
        Invoice_Month_PR:b.Invoice_Month_PR||"",
        Invoice_No_PR:b.Invoice_No||"",Invoice_Date_PR:b.Invoice_Date||"",
        Taxable_Value_PR:b.Taxable_Value||0,IGST_PR:b.IGST||0,CGST_PR:b.CGST||0,SGST_PR:b.SGST||0,Cess_PR:b.Cess||0,

        Invoice_Month_2B:"",Invoice_No_2B:"",Invoice_Date_2B:"",
        Taxable_Value_2B:0,IGST_2B:0,CGST_2B:0,SGST_2B:0,Cess_2B:0,

        Status:"Not in 2B",ThreeB_Status:""
      });
    });
    ...
    twoB.forEach(a=>{
      ...
      if(existing){
        ...
        existing.IGST_2B=a.IGST||0; existing.CGST_2B=a.CGST||0;
        existing.SGST_2B=a.SGST||0; existing.Cess_2B=a.Cess||0;
        ...
        existing.ThreeB_Status="";
      }else{
        map.set(a.recoKey,{
          GSTIN:a.GSTIN||"",Supplier_Name:a.Supplier_Name||"",
          Invoice_Month_2B:a.Invoice_Month_2B||"",
          Invoice_No_PR:"",Invoice_Date_PR:"",
          Taxable_Value_PR:0,IGST_PR:0,CGST_PR:0,SGST_PR:0,Cess_PR:0,
          Invoice_No_2B:a.Invoice_No||"",Invoice_Date_2B:a.Invoice_Date||"",
          Taxable_Value_2B:a.Taxable_Value||0,IGST_2B:a.IGST||0,CGST_2B:a.CGST||0,SGST_2B:a.SGST||0,Cess_2B:a.Cess||0,
          Status:"Not in Books",ThreeB_Status:""
        });
      }
    });
  ...
  const filterFns={includesSome:(row,colId,filterVal)=>!filterVal?.length||filterVal.includes(row.getValue(colId))};
  ...
  const columns=useMemo(()=>[
    ...
    {accessorKey:"Invoice_No_PR",header:"Invoice No. (PR)"},{accessorKey:"Invoice_Date_2B",header:"Invoice Date (2B)"},{accessorKey:"Invoice_Date_PR",header:"Invoice Date (PR)"},
  ...
  const table=useReactTable({
    data:reconciledData,columns,state:{columnFilters},
  ...
  const subtotal=table.getFilteredRowModel().rows.reduce((acc,row)=>{
    ...
    return acc;
  },{Taxable_Value_2B:0,Taxable_Value_PR:0,IGST_2B:0,IGST_PR:0,CGST_2B:0,CGST_PR:0,SGST_2B:0,SGST_PR:0,Cess_2B:0,Cess_PR:0,Diff_Taxable:0,Diff_IGST:0,Diff_CGST:0,Diff_SGST:0,Diff_Cess:0});

  const downloadFormat=()=>{ const headers=["MONTH","GSTIN","Supplier_Name","Invoice_No","Invoice_Date","Taxable_Value","IGST","CGST","SGST","Cess"];
  ...
  const downloadReport=()=>{const order=columns.map(c=>c.accessorKey||c.id);
    const data=table.getFilteredRowModel().rows.map(r=>{const row={};order.forEach(c=>{row[c]=r.original[c];});return row;});
    const ws=XLSX.utils.json_to_sheet(data,{header:order});const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,"Reconciliation");XLSX.writeFile(wb,"gst_2BReco_report.xlsx");};
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="mb-4 flex flex-wrap gap-4 items-center justify-center">
        <label className="cursor-pointer bg-blue-600 text-white px-4 py-2 rounded">Upload GSTR-2B
          <input type="file" accept=".xlsx,.xls" onChange={(e)=>handleFileUpload(e,setData2B)} className="hidden"/>
        </label>
        <label className="cursor-pointer bg-blue-600 text-white px-4 py-2 rounded">Upload Books
          <input type="file" accept=".xlsx,.xls" onChange={(e)=>handleFileUpload(e,setDataBooks)} className="hidden"/>
        </label>

        <button onClick={()=>{
          if(!data2B.length||!dataBooks.length){alert("Upload both files");return;}
